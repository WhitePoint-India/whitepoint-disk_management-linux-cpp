...
//     // Use conv=fdatasync to ensure data is written to disk (more compatible than oflag=direct)
//     // Note: oflag=direct can fail on virtual disks, partitions, and some RAID configurations
//     std::string command = "dd if=/dev/zero of=" + disk.path + " bs=1M status=progress conv=fdatasync";
...
//     // Check if successful - note that "No space left on device" is expected when
//     // dd fills the entire disk, so we treat it as success
//     bool isSuccess = result.success || (result.stdout.find("No space left on device") != std::string::npos);
